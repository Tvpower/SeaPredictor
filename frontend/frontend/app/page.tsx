...
function bboxToAltitude(bbox?: [number, number, number, number]): number {
  ...
  // Altitude is in units of globe radii (~6371 km). A 0.1deg span (~11 km)
  // needs ~0.02 to be readable; pull back ~3x the span otherwise.
  if (span < 0.05) return 0.012;
  if (span < 0.2) return 0.025;
  if (span < 0.5) return 0.07;
  if (span < 1.5) return 0.18;
  return Math.min(1.4, span * 0.8);
}
...
export default function Page() {
  ...
  // Wall-clock seconds it should take to play through the full forecast.
  const PLAYBACK_DURATION_S = 14;
  ...
  // Drive the drift animation with requestAnimationFrame so it stays smooth
  // and pauses when the tab is backgrounded.
  useEffect(() => {
    if (!isPlaying || !forecastPaths) return;
    let rafId = 0;
    ...
    const tick = (now: number) => {
      ...
      setPlaybackProgress((p) => {
        const next = p + dt / PLAYBACK_DURATION_S;
        if (next >= 1) {
          setIsPlaying(false);
          return 1;
        }
  ...
  }, [isPlaying, forecastPaths]);

  const handlePlayPause = useCallback(() => {
    setIsPlaying((p) => {
      // If we're at the end, restart from 0 instead of doing nothing.
      if (!p && playbackProgress >= 1) setPlaybackProgress(0);
      return !p;
    });
  }, [playbackProgress]);
  ...
  const handleRunForecast = useCallback(
    async (params: ForecastParams) => {
      ...
      try {
        ...
        // Auto-play once a fresh forecast lands.
        setPlaybackProgress(0);
        setIsPlaying(true);
      } catch (e) {
      ...
      <section className="globe-stage">
        ...
        <div className="globe-layer">
          <Globe
            ...
            view={view}
            playbackProgress={
              forecastPaths && (isPlaying || playbackProgress < 1)
                ? playbackProgress
                : null
            }
        ...
        <aside className="right-stack">
          <ControlPanel
            ...
            hasForecast={!!forecastPaths}
            isPlaying={isPlaying}
            playbackProgress={playbackProgress}
            forecastDays={forecast?.params.days ?? 7}
