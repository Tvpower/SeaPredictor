...
def _fallback_tile_index_meta() -> dict[str, dict]:
    """Read the lightweight checked-in tile index when raw MARIDA is absent."""
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            if not tile_id or not raw_date:
                continue
            try:
                out[tile_id] = {
...
def _fallback_tile_index_dates() -> dict[str, date]:
    return {tile_id: meta["date"] for tile_id, meta in _fallback_tile_index_meta().items()}
...
def _resolve_dates(
    ...
    out: dict[str, date] = {}
    fallback_dates = _fallback_tile_index_dates()
    idx = None
...
def extract_seeds(
    ...
    if not records:
        return []
    fallback_meta = _fallback_tile_index_meta()
    ...
    for rec in records:
        ...
        if max_prob < min_prob:
            continue
        tile_meta = fallback_meta.get(tile_id)
        if tile_meta is not None:
            latlon = (float(tile_meta["lat"]), float(tile_meta["lon"]))
        else:
            geo = rec.get("geo", {})
