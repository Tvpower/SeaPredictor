...
from src.pipeline.predictions_to_scenes import ensure_scene_cache_from_predictions


# Keep this local so the API can start even when the optional forecast stack
# (OpenDrift/Cartopy) is not installed yet. The forecast endpoint imports the
# heavy drift runner only when a run is requested.
...
ensure_scene_cache_from_predictions(REPO_ROOT)
    ...
    with _drift_lock:
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail=(
                    "forecast stack is not installed in this Python environment. "
                    "Install OpenDrift and its geospatial dependencies, or use a "
                    "Python version with prebuilt Cartopy wheels."
                ),
            ) from e
            ...
            raise HTTPException(
                status_code=503,
                detail=(
                    f"{e} Place OSCAR daily NetCDFs under data/raw/oscar/ "
                    "or data/data/raw/oscar/, or set OSCAR_ROOT to the folder "
                    "containing oscar_currents_interim_YYYYMMDD.nc files."
                ),
            ) from e
